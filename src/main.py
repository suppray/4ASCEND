...
class Application(tk.Tk):
    ...
    def show_main_menu(self):
        self.clear_screen()
        self.current_frame = tk.Frame(self, width=820, height=640)
    ...
    def show_ai_difficulty(self):
        self.clear_screen()
        self.current_frame = AIDifficultyFrame(
            self,
            on_select=self.ai_difficulty_selected,
            on_import_custom=self.import_custom_ai,
            on_back=self.show_game_options
        )
        self.current_frame.pack(expand=True, fill=tk.BOTH)
    ...
    def ai_difficulty_selected(self, diff):
        ...
        self.current_frame = SettingsFrame(
            self,
            on_start=lambda b,w: self.start_ai_game(b, w, difficulty=diff),
            on_back=self.show_ai_difficulty
        )
        self.current_frame.pack(expand=True, fill=tk.BOTH)

    def import_custom_ai(self):
        ...
        module_name = f"custom_ai_{random.randint(1000, 9999)}"
        spec = importlib.util.spec_from_file_location(module_name, file_path)
        if spec is None:
            messagebox.showerror("错误", "无法加载指定的文件。")
            return
        module = importlib.util.module_from_spec(spec)
        try:
            spec.loader.exec_module(module)
        except Exception as e:
            messagebox.showerror("加载错误", f"加载 AI 脚本时出错：{e}")
            return

        if not hasattr(module, 'get_move'):
            messagebox.showerror("接口错误", "AI 脚本中未找到 get_move(board) 函数。")
            return

        # 成功加载，存储自定义 AI 函数
        self.custom_ai_function = module.get_move
        # 进入血量设置
        self.clear_screen()
        self.current_frame = SettingsFrame(
            self,
            on_start=lambda b,w: self.start_ai_game(b, w, custom_ai=self.custom_ai_function),
            on_back=self.show_ai_difficulty
        )
        self.current_frame.pack(expand=True, fill=tk.BOTH)
    ...
    def start_ai_game(self, bh, wh, difficulty=None, custom_ai=None):
        self.clear_screen()
        if custom_ai:
            # 使用自定义 AI
            self.current_frame = GoBoard(
                self, black_hp_init=bh, white_hp_init=wh,
                on_back_to_menu=self.show_main_menu,
                ai_difficulty=custom_ai   # 直接传递函数
            )
        else:
            self.current_frame = GoBoard(
                self, black_hp_init=bh, white_hp_init=wh,
    ...
    def show_rules(self):
        self.clear_screen()
        main_frame = tk.Frame(self, width=820, height=640)
        ...
        rules_path = os.path.normpath(os.path.join(base_dir, "..", "docs", "rules.txt"))
        try:
    ...
    def show_prototype(self):
        self.clear_screen()
        frame = tk.Frame(self, width=820, height=640)
