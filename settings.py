...
class SettingsFrame(tk.Frame):
    def __init__(self, parent, on_start, on_back):
        ...
        tk.Button(self, text="开始对战", font=("Arial", 12), width=15,
                  command=self._validate_and_start).pack(pady=20)
    ...
    def _validate_and_start(self):
        try:
            bh = int(self.black_hp.get())
            wh = int(self.white_hp.get())
        except ValueError:
            messagebox.showerror("输入错误", "血量必须为整数（2~20）。")
            return
        if not (2 <= bh <= 20) or not (2 <= wh <= 20):
            messagebox.showerror("输入错误", "血量范围为 2~20。")
            return
        self.on_start(bh, wh)


class AIDifficultyFrame(tk.Frame):
    def __init__(self, parent, on_select, on_back, on_import_custom=None):
        super().__init__(parent, width=920, height=740)
        ...
        if on_import_custom:
            tk.Button(self, text="导入自定义 AI", font=("Arial", 14), width=15, height=2,
                      command=on_import_custom).pack(pady=10)
